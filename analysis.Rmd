...
# Effects of HA on core body temperature
...
```{r tre, fig.width=5, fig.height=4, fig.cap="**Supplementary Figure S1.** Changes in rectal temperature during 28 days of heat exposure."}
...
# ggsave("output/figure/figure_s1.pdf", p_s1, width = 5, height = 4)
p_s1
```
...
p_beta_bray_28 <- plot_ordination(ps_ev_28, pcoa_bray_28, 
  ...
  theme(legend.title = element_blank(), legend.position = "none") +
  annotate("text",
    label = "bold(Anosim)~~Bray~italic(P)==0.002~~Jaccard~italic(P)==0.004", 
...
p_beta_jaccard_28 <- plot_ordination(ps_ev_28, pcoa_jaccard_28,
  ...
  theme(legend.title = element_blank()) + 
  annotate("text",
    label = "bold(MRPP)~~Bray~italic(P)==0.007~~Jaccard~italic(P)==0.008", 
...
p_s4 <- p_alpha_14 + {p_beta_bray_14 + p_beta_jaccard_14 + plot_layout(ncol = 2)} +
  p_mean_comosition + 
  plot_layout(nrow = 3, heights = c(1.4, 1, 0.8)) + plot_annotation(tag_levels = "A")
...
You should downloaded file 97_otus.fasta from https://gg-sg-web.s3-us-west-2.amazonaws.com/downloads/greengenes_database/gg_13_5/gg_13_5.fasta.gz before run the folling script, unzip it and put file 97_otus.fasta under directory `data/raw`.
...
Overall, the microbial communities present in HA and CR could be distinguished based on their functions (Supplementary Figure S5A).
...
p_s5a <- autoplot(prcomp(pca_in), data = pca_color, colour = "color") + 
  theme_bw() +
  scale_color_npg() +
  theme(legend.position = "none") 
```
...
The predicted KEGG pathway significantly enriched in HA included cell motility, signal transduction, lipid metabolism, metabolism of other amino acids, neurodegenerative diseases, environmental adaption and transport and catabolism (Supplementary Figure S5B, Supplementary Table S5). 
...
p_s5b <- ggplot(enrich_pathway, aes(pathway, lda)) +
    geom_segment(aes(xend = pathway, yend = 0, colour = enrich_group), size = 6) +
  theme_bw() +
  scale_color_npg() +
...
```{r figures5, fig.width=12,fig.height=5,fig.cap="**Supplementary Figure S5.** **(A)** Principal component analysis (PCA) plot comparing the metagenome predictions for KO using PICRUSt of the microbiota of HA and CR subjects. **(B)** Predicted KEGG Pathways differentially abundant between HA and CR subjects."}
p_s5 <- p_s5a + p_s5b + 
  plot_layout(ncol = 2) +
  plot_annotation(tag_levels = "A")
ggsave("output/figure/figure_s5.pdf", p_s5, width = 12, height = 5)
p_s5
```
...
Among all the phenotypes, the relative abundance of the Aerobic was significantly increased  and the stress tolerant was significantly decreased after HA (Figure 4).

```{r bugbase-phenotype, fig.width=5,fig.height=4,fig.cap="**Figure 4.** Phenotypic prediction based on BugBase analysis. BugBase is a bioinformatics tool that infers community-wide phenotypes, and predict phenotypic differences from 16S rRNA sequence data. Among all the phenotypes, the relative abundance of the Aerobic was significantly increased and the stress tolerant was significantly decreased after HA subjects. Statistical significance was determined by Pairwise Wilcoxon tests. FDR-corrected pairwise P value ranges are: * P < 0.05."}
...
reads_path <- "/Volumes/数据备份/诺禾致源结果/16S/第二阶段/P101SC18030731-01-B8-3_result/01.CleanData"
